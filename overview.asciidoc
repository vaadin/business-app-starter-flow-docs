...
A starter for building responsive business web apps. It consists of a collection of components, view templates, theming utilities and best practises that can help teams save more than a week of development time at the start of a new project.
...
All of the Business App Starter's views extend either `ViewFrame` or `SplitViewFrame`. They simplify the process of creating views that contain a header, scrollable content and footer.

The only difference between them is that `SplitViewFrame` houses an additional *details* slot.
...
* Place detailed information in the *details slot*, e.g. when creating master-detail views. (`SplitViewFrame`)
