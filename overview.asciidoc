...
A common layout where the outer header is the most prominent element. The view uses a view header to show its title.
...
This layout puts more focus on sidebar navigation. Suitable for complex hierarchical navigation structures. The view sports a footer that houses view-specific actions.
