...
Combinations can be created with `createButton(ButtonVariant...)`.
...
|`UIUtils.createLabel(TextColor.PRIMARY, "Primary label");`

|Size
|`createLabel(FontSize, String)`
|`UIUtils.createLabel(FontSize.S, "Small label");`

|Size & color
|`createLabel(FontSize, TextColor, String)`
|`UIUtils.createLabel(FontSize.S, TextColor.PRIMARY, "Small primary label");`
...
|`UIUtils.createH1Label("H1 label");`
|===
...
|Initialises a floating action button positioned in the bottom right corner of its container. Used for primary actions.
...
[source,java]
----
// A medium-sized tertiary icon.
UIUtils.createIcon(IconSize.M, TextColor.TERTIARY, VaadinIcon.HOME);
----
