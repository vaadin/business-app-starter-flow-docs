...
== Properties
[%hardbreaks]
Whitespace can be applied using three different _properties_: `Margin`, `Padding` and `Spacing`.
...
== Sizes
[%hardbreaks]
There are five different _sizes_: `XS`, `S`, `M` (default), `L` and `XL`.

== Directions
[%hardbreaks]
And nine different _directions_: `BOTTOM`, `LEFT`, `RIGHT`, `TOP`, `HORIZONTAL`, `VERTICAL`, `TALL`, `UNIFORM` (default) and `WIDE`.
