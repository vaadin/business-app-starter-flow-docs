...
All of the Business App Starter's views extend either `ViewFrame` or `SplitViewFrame`. These classes simplify the process of creating views with a header, scrollable content and footer. The only difference between them is that `SplitViewFrame` houses an additional *details* slot. This slot comes in handy when creating master-detail views. All slots in `ViewFrame` and `SplitViewFrame` are optional.
...
* Place detailed information in the *details slot*. (`SplitViewFrame` only.)
