...
All of the Business App Starter's views extend either `ViewFrame` or `SplitViewFrame`. These classes simplify the process of creating views with a header, scrollable content and footer. `SplitViewFrame` contains an additional details slot, used when creating master-details view.
