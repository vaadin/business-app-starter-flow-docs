...
The application supports two different navigation modes: default and tabbed.

The default is the more traditional approach where each link change the currently active view. In tabbed navigation mode an additional bar is added to UI and each navigation event changes the active tab. It enables a browser-like functionality by having multiple views open simultaneously.
...
All of the Business App Starter's views extend either `ViewFrame` or `SplitViewFrame`. `ViewFrame` simplifies the process of implementing views consisting of a header, scrollable content and footer. `SplitViewFrame` does the same with an addition of a details section used when creating master-details view. All slots are optional.
